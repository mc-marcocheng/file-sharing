...
app.layout = html.Div([
    html.H3(id='cur'),
    dcc.Slider(id='slider', value=4, min=0, max=8, step=0.1),
    html.H3(id='iter'),
...
def make_fig(cur_x, iter):
    ...
    saved = cur_x
    f = lambda x: np.sign(x-4)*np.sqrt(np.sign(x-4)*(x-4))
    df = lambda x: 1/(2*np.sqrt((x-4)*np.sign(x-4)))
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=(xx := np.linspace(0,8,500)), y=f(xx), mode='lines', line=dict(color='Green'), name="f(x)"))
    for i in range(iter):
        if saved != 4:
        ...
        if saved == 4:
            fig.add_trace(go.Scatter(x=[4,4], y=[-100,100], mode='lines', line=dict(color=colors[i]), name="Iteration "+str(i+1)))
        else:
